    ...
    def joy_callback(self, msg):
        ...
        if(float(roll) == 0.0 and float(pitch) == 0.0):
            ...
            if(float(roll) > 0.2):
                ...
                print("Moving LEFT")
                if(roll != 1):
                    speed = round(float(roll), 2)
                self.value = 4 + abs(speed)
                print(self.value)
        ...
        # PROVA
        self.arduino.write(struct.pack('f', self.value))

        self.arduino.flush()
        #self.arduino.write(self.value)
        time.sleep(0.05)
